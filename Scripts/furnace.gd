...
func _ready() -> void:
	...
	# Make the Furnace clickable for recipe selection UI
...
# --- RECIPE UI SELECTION & COPY/PASTE ---
...
func _try_start_smelting() -> void:
	if is_smelting or selected_recipe == "" or input_buffer.is_empty():
		return
	...
	if input_buffer.count(required_item_id) >= required_amount:
		...
		smelting_timer.wait_time = craft_duration
